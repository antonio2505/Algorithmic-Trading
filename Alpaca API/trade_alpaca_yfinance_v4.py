...
tickers = ['M', 'MTB', 'AER', 'AMC', 'AMD', 'ARKK', 'ASML', 'BHP', 'BX', 'CFG', 'COIN',
 'CRM', 'XBI', 'DASH', 'DOCU', 'EPAM', 'ET', 'ETSY', 'FIVE', 'GFS', 'GLOB', 'HCA', 'HD', 'KRE', 
 'LABU', 'LAC', 'MU', 'NET', 'NVDA', 'FB',  'AMZN',  'INTC',  'MSFT',  'AAPL',  'GOOG',  'CSCO',  'CMCSA',  'ADBE',  'NVDA',  
  'NFLX',  'PYPL',  'AMGN',  'AVGO',  'TXN',  'CHTR',  'QCOM',  'GILD',  'FISV',  'BKNG',  'INTU',  'ADP',  'CME',  'TMUS',  'MU']
...
def hist_data(symbols, timeframe="15m", start='',end=''):
    bars = {}
    for symbol in symbols:
        temp = pd.DataFrame()
        temp = yf.download(symbol,start,end, interval=timeframe)
        temp = temp.rename(columns={"Close":"close","High":"high","Low":"low"})
...
def main():
    global stoch_signal
    historicalData = hist_data(tickers,timeframe='15m',start=start, end=end)
    RSI(historicalData)
    BB(historicalData)
    ...
    for position in positions:
        try:
            for ticker in tickers:
                ...
                elif position.symbol == ticker and float(api.get_position(ticker).unrealized_intraday_pl) <= -200:
                    api.close_position(ticker)
